...
import { sideTopItemList, sideMainItemList, sideButItemList, PlusIcons } from './icon.js';
...
const MenuComponent = () => {
    ...
    const renderList = () => {
        const list = sideTopItemList.map(item => {
            return (
                <div className="menu__navOut" style={{ width: isOpen ? "240px" : "44px" }}>
                    <NavLink NavLink className={isOpen ? "nav-link-w" : "nav-link"} to={item.link} style={{ justifyContent: isOpen ? "start" : "center" }} exact>
                        <div title={item.title} className="icon">{item.icon}</div>
                        <div onClick={toggle} style={{ display: isOpen ? "block" : "none" }} className='menu-item'>{item.title}</div>
                    </NavLink>
                </div>
            );
        })
    ...
    const renderListMain = () => {
        const list = sideMainItemList.map(item => {
            return (
                <div className="menu__navOut" style={{ width: isOpen ? "240px" : "44px" }}>
                    <NavLink className={isOpen ? "nav-link-w" : "nav-link"} NavLink to={item.link} style={{ justifyContent: isOpen ? "start" : "center" }} exact>
                        <div title={item.title} className="icon">{item.icon}</div>
                        <div onClick={toggle} style={{ display: isOpen ? "block" : "none" }} className='menu-item'>{item.title}</div>
                    </NavLink>
                </div>
    ...
    const renderListBut = () => {
        const list = sideButItemList.map(item => {
            return (
                <div className={isOpen ? "nav-link-w" : "nav-link"} style={{ width: isOpen ? "240px" : "44px" }}>
                    <NavLink NavLink to={item.link} style={{ justifyContent: isOpen ? "start" : "center" }} className="nav-link" exact>
                        <div title={item.title} className="icon">{item.icon}</div>
                        <div onClick={toggle} style={{ display: isOpen ? "block" : "none" }} className='menu-item'>{item.title}</div>
                    </NavLink>
                </div>
            ...
            <div className='menu' style={{ width: isOpen ? "265px" : "66px" }}>
                <div className='logo' style={{ justifyContent: isOpen ? "space-between" : "center" }}>
                    ...
                    <div className="bars">
                        ...
                        />
                        <Menu sx={{ color: "#fff" }} onClick={toggle} style={{ display: isOpen ? "none" : "block" }} />
                    </div>
                </div>
                <div className="scrollbox"
                    style={{ padding: isOpen ? "0" : "0" }}
                >
                    {renderList()}
                    <div><hr style={{ width: isOpen ? "230px" : "44px" }} className='hr-bar menu__navOut'></hr></div>
                    {renderListMain()}
                    <button className='btnPlus' style={{ justifyContent: isOpen ? "start" : "center" }}>
                        <div className="nav-link btn-link">

                            {<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                <path d="M7.66864 16C7.11635 16 6.66864 15.5523 6.66864 15V1C6.66864 0.447715 7.11635 0 7.66864 0H8.3215C8.87379 0 9.3215 0.447715 9.3215 1V15C9.3215 15.5523 8.87378 16 8.3215 16H7.66864ZM1 9.3215C0.447715 9.3215 0 8.87378 0 8.3215V7.66864C0 7.11635 0.447715 6.66864 1 6.66864H15C15.5523 6.66864 16 7.11636 16 7.66864V8.3215C16 8.87379 15.5523 9.3215 15 9.3215H1Z" fill="#141516" />
                            </svg>}

                            <div onClick={toggle} style={{ display: isOpen ? "block" : "none" }} className='menu-item createTitle'>{'Создать заказ'}</div>
                        </div>
                    </button>
                </div>
                <div style={{ justifyContent: isOpen ? "flex-end" : "flex-end" }} className="nav-footer" exact>
                    {renderListBut()}
                </div>
            </div>
        </div>
